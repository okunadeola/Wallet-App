...
export class RegisterComponent implements OnInit {
  ...
  register() {
    if (
      ...
      form.account = Math.floor(Math.random() * 10000000000);
      form.balance = 0;
      form.loanArr = [];
      form.history = [];
      form.message = [];
      form.walletCatalogue = [];
      ...
      if (localStorage.getItem('walletRegister') !== null) {
        ...
        if (finder) {
          ...
          if (localStorage.getItem('walletRegister') === null) {
            ...
            let data: any = localStorage.getItem('walletRegister');
            let newData = JSON.parse(data);
            newData.push(this.registerLog[this.registerLog.length - 1]);
            localStorage.setItem('walletRegister', JSON.stringify(newData));
          }
